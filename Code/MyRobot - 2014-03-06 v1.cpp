...
private:
	...
	void AutoShot(double distance = 1, double power = 1, bool instantRetract = false) {
		isShooting = true;
		myRobot->MecanumDrive_Cartesian(0, 0, 0, 0);
		while(shooterPot->GetVoltage()*100 < 205) {
			upDown->Set(-0.75);
			roller->Set(0.5);
		}
		upDown->Set(0);
		...
		if(shooterSwitch->Get()) {
			...
			temp = GetTime();
			while(shooterEncoder->GetDistance() < fullShot*distance && GetTime()-temp < 0.4/pow(power, 1/3)) {
				shooter->Set(power);
			}
			shooter->Set(0);
			if(!instantRetract)
...
void RobotDemo::TeleopPeriodic() {
	...
	if(DriverStation::GetInstance()->GetDigitalIn(1)) {
		...
		if(ManipulatorPad->GetRawButton(3)){
			...
			if(fabs(ManipulatorPad->GetRawAxis(4)) > .15)
				roller->Set(pow(ManipulatorPad->GetRawAxis(4), 3));
			else
				roller->Set(0);
		}
		...
		*/
		...
		//if(!(DriverPad->GetRawButton(6) || DriverPad->GetRawButton(8))) {
		//	isShooting = false;
		//}
		...
		if(DriverPad->GetRawButton(9)){
			shooterEncoder->Reset();
		}	
		
		if (DriverPad->GetRawAxis(6) > 0.5) { //test ''
			roller->Set(1);   // this is  test delete if broken code 
		} else if (DriverPad->GetRawAxis(6) < -0.5) { //test ''
			roller->Set(-1);   // this is  test delete if broken code 
		} else {
			roller->Set(0);
		}
		...
		if(DriverPad->GetRawButton(5) && shooterSwitch->Get()) {
			shooter->Set(-0.75);
		} else if(DriverPad->GetRawButton(6) && shooterEncoder->GetDistance() < fullShot){
			shooter->Set(1);
		} else if(DriverPad->GetRawButton(8)){
			shooter->Set(0.55); //Much tired.  Very code.  Wow. (y)
		} else if(DriverPad->GetRawButton(7)) {
		...
		if((DriverPad->GetRawButton(7) || DriverPad->GetRawButton(5)) && !shooterSwitch->Get())
			shooterEncoder->Reset();
		*/
		...
		if(!isShooting && !isElTorroInOperation && isElTorroReset) {
			if(DriverPad->GetRawButton(6)) {
				...
				if(DriverStation::GetInstance()->GetDigitalIn(2)) {
					...
					AutoShot(DriverStation::GetInstance()->GetAnalogIn(1)/5, DriverStation::GetInstance()->GetAnalogIn(2)/5);
				}
			} else if(DriverPad->GetRawButton(8)) {
				//isShooting = true;
				AutoShot(0.5, 0.55);
