...
private:
	...
	void addSamples(double x, double y, double z) {
		...
		while(index < samples-1) {
			joyXsamples[index] = joyXsamples[index+1];
			joyYsamples[index] = joyYsamples[index+1];
			joyZsamples[index] = joyZsamples[index+1];
			index++;
		}
		...
		index = 0;
		while(index < samples) {
			joyXsamples[samples] += (joyXsamples[index]-joyXsamples[samples])/(index+1);
			joyYsamples[samples] += (joyYsamples[index]-joyYsamples[samples])/(index+1);
			joyZsamples[samples] += (joyZsamples[index]-joyZsamples[samples])/(index+1);
			index++;
		}
	}
	...
	void GeneralPeriodic() {
		...
		index = 0;
		while(index < 6) {
			sprintf(distance, "%f", stick2->GetRawAxis(index+1));
			userMessages->PrintfLine((DriverStationLCD::Line) index, distance);
			index++;
		}
		userMessages->UpdateLCD();
		//DriverStation::GetInstance()->GetDigitalIn();
	}
...
public: 
	RobotDemo() {
		...
		roller = new Talon(5);
		shooter = new Talon(6);
		upDown = new Victor(7);   // up down for el torro lift delete if broken code 
...
void RobotDemo::AutonomousInit() {
	...
	*/
	//visionTracking.Autonomous();
	
/*	if(visionTracking.hotOrNot())
		shoot();
	else
		wait(1);
}
...
void RobotDemo::TeleopPeriodic() {
	...
	*/
	...
	/*		else if(stick->GetRawButton(3))
			...
			if(stick2->GetRawButton(3)){
				...
				if(fabs(stick2->GetRawAxis(4)) > .15)
			...
			if (stick2->GetRawButton(1)) { //test ''
				upDown->Set (-0.5);   // this is  test delete if broken code 
			} else if (stick2->GetRawButton(4)) { //test ''
			...
			if(stick2->GetRawButton(5)) {
				shooter->Set( pow ( limit( -( atan( ( shooterEncoder->GetDistance()-10/*+490*/ ) / 60 ) ) ), 3) *0.50);
			} else if(stick2->GetRawButton(6) ){
				shooter->Set(1);
			} else if(stick2->GetRawButton(8)){
				shooter->Set(0.25); //Much tired.  Very code.  Wow. (y)
			} else if(stick2->GetRawButton(7)) {
				shooter->Set(-0.55);
			} else {
				shooter->Set(0);
			}
		/*&& shooterEncoder->GetDistance() > -20
	...
	*/
	...
	/*		else if(stick->GetRawButton(3))
			...
			if (stick->GetRawAxis(6) > 0.5) { //test ''
				roller->Set(1);   // this is  test delete if broken code 
			} else if (stick->GetRawAxis(6) < -0.5) { //test ''
				roller->Set(-1);   // this is  test delete if broken code 
			} else {
				roller->Set(0);
			}
