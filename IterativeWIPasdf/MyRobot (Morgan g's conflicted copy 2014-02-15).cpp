...
private:
	...
	Talon *roller;
	Talon *shooter;
	Victor *up_down;//TEST! if broken, delete
...
public:
	...
	{	
		...
		FR = new Talon(3);
		RR = new Talon(4);
		up_down = new Victor(7);//test. if broken, delete.
		roller = new Talon(5);
		shooter = new Talon(6);
...
*/
		...
		if(stick->GetRawButton(4)){
			...
			if(fabs(stick->GetRawAxis(4)) > .15)
				...
				roller->Set(0);
			if(button->GetRawButton(5)){
						up/down->Set(1);//this is ONLY a test. if broken code, delete.
		}
		if(stick->GetRawAxis(7) < 0){
			shooterEncoder->Reset();
		}
